...
@endif
        <form action="{{ route('roles.store') }}" method="post">
            ...
            <div>
                ...
                <input type="text" name="name" id="product-name-ar" class="mt-1 block w-full p-2 border border-gray-300 rounded-md dark:bg-gray-600 dark:border-gray-500 dark:text-gray-100">
            </div>
                ...
                <div class="overflow-x-auto" style="padding:10px">
                    <table class="min-w-full bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                        ...
                        <tbody id="products-table-body" >
                            ...
                            @php $index = 0 ; @endphp
                            @foreach (config('abilities') as $permission => $valueOfPermission)
                                ...
                                <tr style="height: 40px;border-bottom:1px solid rgb(222, 219, 219)">
                                    ...
                                    <td style="text-align: center">
                                        ...
                                        <input type="radio" name="permissions[{{ $permission }}]" value="allow" checked>
                                    </td>
                                    <td style="text-align: center">
                                        <label for="check_{{ $index }}">Deny</label>
                                        <input type="radio" name="permissions[{{ $permission }}]" value="deny">
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    ...
                    <button type="submit" class="edit-product-btn bg-blue-500 text-white px-3 py-1 rounded-md text-sm hover:bg-blue-600 transition-colors duration-200 ml-2 rtl:mr-2">Create</button>
                </div>
            </section>
        </form>
